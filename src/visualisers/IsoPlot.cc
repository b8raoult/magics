...
public:
    ...
    void push_back(CellBox* box) { AutoVector<CellBox>::push_back(box); }
...
public:
    IsoProducerData(bool shading, IsoPlot& parent, CellBox& cell) : shading_(shading), parent_(parent), cell_(cell) {
        more_ = true;
    }
    bool shading_;
    IsoPlot& parent_;
...
void IsoPlot::visit(Data& data, LegendVisitor& legend) {
    ...
    switch (legend.legendType()) {
        ...
        case LegendMethod::DISJOINT: {
            ...
            if (shading_->hasLegend())
                return;
            if (rainbow_) {
                for (vector<double>::iterator level = levelSelection_->begin(); level != levelSelection_->end();
                     ++level) {
...
GridArray::GridArray(MatrixHandler& data, IntervalMap<int>& range, const Transformation& transformation, int width,
                     int height, float resol, const string& technique) :
    CellArray(data, range) {
