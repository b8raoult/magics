...
void ArrowPlotting::operator()(bool north, const PaperPoint& point, double x, double y, double val) {
    ...
    if (!levels_->empty() && (value < levels_->front() || value > levels_->back()))
        return;
    if (speed < min_speed_ || speed > max_speed_)
        return;

    if ((*this->calm_)(point, x, y))
        return;
...
struct SortHelper {
    SortHelper() {}
    ~SortHelper() {}
    MAGICS_NO_EXPORT bool operator()(const Arrow* first, const Arrow* second) { return first->size() > second->size(); }
};
