...
void SimplePolylineVisualiser::setup() {
    ...
    for (int i = 0; i < max; i++) {
        ...
        if (colour == colour_list_.end())
            colour = (colour_policy_ == ListPolicy::M_CYCLE) ? colour_list_.begin() : --colour;
    }
    ...
    for (int i = 0; i < max; i++) {
        ...
        if (style == style_list_.end())
            style = (style_policy_ == ListPolicy::M_CYCLE) ? style_list_.begin() : --style;
    }
    ...
    for (int i = 0; i < max; i++) {
        ...
        if (thickness == thickness_list_.end())
            thickness = (thickness_policy_ == ListPolicy::M_CYCLE) ? thickness_list_.begin() : --thickness;
    }
...
LineStyle SimplePolylineVisualiser::style(const CustomisedPoint& point) {
    LineStyle solid                       = LineStyle::M_SOLID;
