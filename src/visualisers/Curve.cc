...
void setHandler(const vector<string>& keys, vector<T>& values, ListPolicy policy, const T& def,
                map<string, O>& handler) {
    if (values.empty())
    ...
    for (vector<string>::const_iterator key = keys.begin(); key != keys.end(); ++key) {
        ...
        value++;
        if (value == values.end())
            value = (policy == ListPolicy::M_CYCLE) ? values.begin() : --value;
    }
}
...
bool Curve::missing(CustomisedPoint& point) const {
    ...
    if (x > this->x_above_)
        return true;
    if (same(x, this->x_below_))
        return true;
    if (same(x, this->x_above_))
        return true;
    ...
    if (y > this->y_above_)
        return true;
    if (same(y, this->y_below_))
        return true;
    if (same(y, this->y_above_))
        return true;
    return false;
}
    ...
    while (point != points.end()) {
        ...
        if (!this->missing(**point)) {
            if (last_out) {
                ...
                result = (method == missingMethods_.end()) ? ignore(last, missing.front(), missing, task)
...
public:
    CurveEntry(const string& legend, Curve& curve) : LegendEntry(legend), curve_(curve) {}
    void set(const PaperPoint& point, BasicGraphicsObjectContainer& legend) { curve_.set(point, legend, *this); }
...
bool Curve::join(const PaperPoint& p1, const PaperPoint& p2, const vector<PaperPoint>&,
...
bool Curve::drop(const PaperPoint& p1, const PaperPoint& p2, const vector<PaperPoint>& points,
                 BasicGraphicsObjectContainer& task) {
    magics::Polyline* curve = new magics::Polyline();
    (*curve).setColour(*missing_colour_);
    (*curve).setLineStyle(missing_style_);
