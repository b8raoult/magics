...
void GDDriver::open() {
    ...
    if (gdFontCacheSetup() != 0)
        MagLog::error() << "GDDriver::readFonts() --> Cannot initialize font cache!\n";
#endif
...
void GDDriver::close() {
    ...
    if (jpg_)
        MagLog::info() << "JEPG is not a good format for scientific plots. Please consider using GIF or PNG.\n";
...
MAGICS_NO_EXPORT void GDDriver::startPage() const {
    ...
    if (white == -1)
        MagLog::error() << "GDDriver: Could NOT allocate background colour!\n";
...
MAGICS_NO_EXPORT void GDDriver::endPage() const {
    ...
    if (animated_) {
        if (currentPage_ == 0)
            gdImageGifAnimAdd(currentImage_, outFile_, 1, 0, 0, delay_, 1, NULL);
        else
            gdImageGifAnimAdd(currentImage_, outFile_, 1, 0, 0, delay_, 1, Images_[currentPage_ - 1]);
    }
    else
#endif
#endif
    {
        ...
        if (gif_) {
#ifndef MAGICS_GIF
            MagLog::warning() << "GIF and Animated GIF are not supported in this version (Too old version of GD "
                                 "library!). You get a PNG instead.";
            gif_ = false;
            png_ = true;
#else
...
MAGICS_NO_EXPORT void GDDriver::renderText(const Text& text) const {
    ...
    for (; niceText < niceTextEnd; niceText++) {
        ...
        if (!fontfile) {
            ...
            MagLog::warning() << "GDDriver (pretty): TTF font file " << ttf << " could not be found! Font " << new_font
        ...
        int bbx[8];
        char* err = gdImageStringFT(NULL, &bbx[0], 0, (char*)ttf.c_str(), fontSize, 0., 10, 10,
                                    (char*)(*niceText).text().c_str());
        if (err)
            MagLog::error() << "GDDriver::renderText() -> Could not determine text bounding box for >"
                            << (*niceText).text() << "< !\n";
    ...
    for (; niceText < niceTextEnd; niceText++) {
        ...
        if (currentFont_ != ttf) {
            ...
            if (!fontfile) {
                const string new_font = lowerCase("sansserif_" + style);
                MagLog::warning() << "GDDriver (pretty): TTF font file " << ttf << " could not be found! Font "
        ...
        for (unsigned int nT = 0; nT < noTexts; nT++)  // for all sting COORDINATES
        {
            ...
            if (text.getBlanking()) {
                err = gdImageStringFT(NULL, &bbx[0], 0, (char*)ttf.c_str(), fontSize, LastAngle_, x0, y0,
                                      (char*)(*niceText).text().c_str());
                if (!err) {
                    ...
                    MagLog::error() << "GDDriver: gdImageStringFT -> " << err << " -> NO blanking!!!" << endl;
            }
            err = gdImageStringFT(currentImage_, &bbx[0], currentColourIndex_, (char*)ttf.c_str(), fontSize, LastAngle_,
                                  x0, y0, (char*)(*niceText).text().c_str());
            if (err) {
                MagLog::error() << "GDDriver: gdImageStringFT -> " << err << endl;
                gdImageString(currentImage_, gdFontGetLarge(), x0, y0, (unsigned char*)(*niceText).text().c_str(),
                              currentColourIndex_);
            }
            delete[] err;
        }
...
MAGICS_NO_EXPORT void GDDriver::circle(const MFloat x, const MFloat y, const MFloat r, const int s) const {
    ...
    if (s < 8) {
        ...
        gdImageFilledArc(currentImage_, cx, cy, rx, ry, 0, 360, currentLineStyleIndex_, gdArc);
    ...
    for (int i = height - 1; i >= 0; i--) {
        for (int j = 0; j < width; j++) {
            ...
            if (!(lt[c] == "undefined")) {
                ...
                gdImageFilledRectangle(this->currentImage_, wx, wy, static_cast<int>(wx + dx),
                                       static_cast<int>(wy + dy), kk);
            }  // point has colour
        }
    }
    return true;
