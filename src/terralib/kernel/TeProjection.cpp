...
TeProjection* TeProjectionFactory::make(const TeProjectionParams& par) {
    string punits;
    if (par.units.empty())
        punits = "Meters";
    else
    ...
    if (par.name == "UTM")
        return new TeUtm(par.datum, par.lon0, par.lat0, par.offx, par.offy, punits, par.scale, par.hemisphere);

    if (par.name == "LambertConformal")
        return new TeLambertConformal(par.datum, par.lon0, par.lat0, par.offx, par.offy, par.stlat1, par.stlat2,
                                      punits);

    if (par.name == "Albers")
        return new TeAlbers(par.datum, par.lon0, par.lat0, par.offx, par.offy, par.stlat1, par.stlat2, punits);
    ...
    if (par.name == "Mercator")
        return new TeMercator(par.datum, par.lon0, par.lat0, par.offx, par.offy, par.stlat1, punits);
    ...
    if (par.name == "Satellite")
        return new TeSatelliteProjection(par.datum, par.offx, par.offy, par.pri, par.prj, par.pis, par.pjs, par.lat0,
                                         par.lon0, par.prs, par.pscn, par.pyaw);
...
TeProjectionParams TeProjection::params() const {
...
********************************************************************/
void TeProjection ::ChangeLL(double& lon1, double& lat1) {
    double equad1,  // Squared eccentricity - datum 1
        ...
        y2, z2, d, lat2,  // Ancillary variable
        lon2;
    double Pflt = GPdatum.flattening();
    double Prd  = GPdatum.radius();

    if (GPdestination->GPdatum.radius() == 0.)
        return;
...
TeCoord2D TeUtm ::LL2PC(TeCoord2D& ptll) {
    double k0,     // Scale factor
        equad,     // Squared eccentricity
        n,         // Great normal of ellipsoid
        elinquad,  // ancillary variables
        aux1, aux2, aux3, aux4, aux5, aux6, aux7, aux8, aux9, aux10, aux11, aux12, aux13, t, c, ag, m, ptllx, ptlly;
...
TeCoord2D TeUtm ::PC2LL(TeCoord2D& ptpc) {
    double k0,     // Scale factor
        equad,     // Squared eccentricity
        n1,        // Great normal of ellipsoid
        elinquad,  // Ancillary variables
        e1, aux1, aux2, aux3, aux4, aux5, m, mi, aux6, aux7, aux8, lat1, c1, t1, quoc, r1, d, aux9, aux10, aux11, aux12,
        ptpcx, ptpcy;
...
********************************************************************/
TeCoord2D TeMercator ::LL2PC(TeCoord2D& ptll) {
...
********************************************************************/
TeCoord2D TeMercator ::PC2LL(TeCoord2D& ptpc) {
    double equad,  // Squared eccentricity
        pi,        // PII value
...
********************************************************************/
TeCoord2D TeLambertConformal ::LL2PC(TeCoord2D& ptll) {
    double equad,  // Squared eccentricity
        e,         // Ancillary variables
        m1, m2, aux1, aux2, aux0, t1, t2, t0, n, efe, ro0, aux, t, ro, teta, ptllx, ptlly;
 ...
 ********************************************************************/
TeCoord2D TeLambertConformal ::PC2LL(TeCoord2D& ptpc) {
    double equad,  // Squared eccentricity
        pi,        // PI value
        e, m1, m2, aux1, aux2, aux0, t1, t2, t0, n, efe, ro0, ro, teta, t, xx, aux3, aux4, aux5, ptpcx, ptpcy;
...
********************************************************************/
TeCoord2D TePolyconic ::LL2PC(TeCoord2D& ptll) {
    double equad,  // Squared eccentricity
        n,         // Great normal of ellipsoid
        aux01,     // Ancillary variables
        aux02, aux03, aux04, m0, aux1, aux2, aux3, aux4, m, e, ptllx, ptlly;
    ...
    if (ptlly == 0.) {
        ...
        aux2 =
            (3. * equad / 8 + 3. * equad * equad / 32. + 45. * equad * equad * equad / 1024.) * sin((double)2 * ptlly);
...
********************************************************************/
TeCoord2D TePolyconic ::PC2LL(TeCoord2D& ptpc) {
    double A, B, C,
        equad,  // Squared eccentricity
        aux01,  // Ancillary variables
        aux02, aux03, aux04, m0, mn, mnl, ma, cp, lat1, lat2, aux05, aux06, aux07, aux08, aux09, aux10, aux11, aux12,
        aux21, aux22, aux23, aux24, ptpcx, ptpcy;
    ...
    if (ptpcy == (-m0)) {
        ...
        do {
            ...
            aux22 = (3. * equad / 8. + 3. * equad * equad / 32. + 45. * equad * equad * equad / 1024.) *
                    sin((double)2 * lat2);
            ...
            aux06 = 2. * (3. * equad / 8. + 3. * equad * equad / 32. + 45. * equad * equad * equad / 1024.) *
                    cos((double)2 * lat2);
...
********************************************************************/
TeCoord2D TeLatLong ::LL2PC(TeCoord2D& ptll) {
    return TeCoord2D(ptll.x() * TeCRD, ptll.y() * TeCRD);
}

// FAMI
void TeLatLong ::LL2PC(double xi, double yi, double& xo, double& yo) const {
    xo = xi * TeCRD;
    yo = yi * TeCRD;
    return;
}
...
********************************************************************/
TeCoord2D TeLatLong ::PC2LL(TeCoord2D& ptpc) {
    double ptpcx, ptpcy;
...
// FAMI
void TeLatLong ::PC2LL(double xi, double yi, double& ptpcx, double& ptpcy) {
    ptpcx = xi * TeCDR;
    ptpcy = yi * TeCDR;
...
********************************************************************/
TeCoord2D TeAlbers ::LL2PC(TeCoord2D& ptll) {
    double equad,  // Squared eccentricity
        e,         // Ancillary variables
        m1, m2, aux1, aux10, aux11, aux12, aux2, aux20, aux21, aux22, q0, q1, q2, q, n, c, ro0, teta, ro, ptllx, ptlly;
...
********************************************************************/
TeCoord2D TeAlbers ::PC2LL(TeCoord2D& ptpc) {
    double equad,  // Squared eccentricity
        e,         // Eccentricity
        m1,        // Ancillary variable
        m2, aux10, aux11, aux12, aux20, aux21, aux22, q0, q1, q2, n, c, ro0, ro, q, aux, beta, aux1, aux2, aux3, teta,
        ptpcx, ptpcy;
...
********************************************************************/
TeCoord2D TeMiller ::LL2PC(TeCoord2D& ptll) {
    double pi,         // PI value
...
********************************************************************/
TeCoord2D TeMiller ::PC2LL(TeCoord2D& ptpc) {
...
********************************************************************/
TeCoord2D TeSinusoidal ::LL2PC(TeCoord2D& ptll) {
...
********************************************************************/
TeCoord2D TeSinusoidal ::PC2LL(TeCoord2D& ptpc) {
...
********************************************************************/
TeCoord2D TeCylindricalEquidistant ::LL2PC(TeCoord2D& ptll) {
    double ptllx, ptlly;
...
********************************************************************/
TeCoord2D TeCylindricalEquidistant ::PC2LL(TeCoord2D& ptpc) {
    double ptpcx, ptpcy;
...
********************************************************************/
TeCoord2D TePolarStereographic ::LL2PC(TeCoord2D& ptll) {
    double k0,  // Scale factor
        e,      // Eccentricity
...
// FAMI
void TePolarStereographic ::LL2PC(double xi, double yi, double& xo, double& yo) const {
    double k0,  // Scale factor
        e,      // Eccentricity
        pi,     // Auxillary variables
        lon0,   // auxilliary origin longitude
...
void TePolarStereographic ::LL2PC(ostream& java) const {
    ...
    // java << "\tyll = xx + " << aux3 << "*Math.sin(2*xx) + " << aux4 << "*Math.sin(4*xx) + "<< aux5 <<
    // "*Math.sin(6*xx);\n";
...
********************************************************************/
TeCoord2D TePolarStereographic ::PC2LL(TeCoord2D& ptpc) {
    double k0,  // Scale factor
...
*************************************************************************/
TeCoord2D TeSatelliteProjection::LL2PC(TeCoord2D& ptll) {
...
************************************************************************/
TeCoord2D TeSatelliteProjection::PC2LL(TeCoord2D& ptpc) {
    double dl, dp, x, y, z, x1, x2, equad, n, d, sn, a, b, c, Rd2, Rm, Rm2, Rs2, v, ptpcx, ptpcy, yla, xlo, resx, resy;
...
TeProjectionParams TeSatelliteProjection::params() const {
    TeProjectionParams par;
...
TeDatum TeGetDatumFromProj4(const string& proj4) {
    ...
    for (unsigned int i = 0; i < proj4.size(); i++) {
        ...
        if (proj4[i] == ' ')
            value = proj4.substr(pos + 1, i - pos - 1);
        else
            value = proj4.substr(pos + 1, i - pos);
    ...
    while (*pszInput != '\0') {
        if (*pszInput == '"') {
            isInQuote = !isInQuote;
                 ...
                 (*pszInput == '[' || *pszInput == ']' || *pszInput == ',' || *pszInput == '(' || *pszInput == ')')) {
            break;
        }
        else if (!isInQuote && (*pszInput == ' ' || *pszInput == '\t' || *pszInput == 10 || *pszInput == 13)) {
