...
protected:
    void print(ostream& out) const { out << "[" << text_ << " (" << font_ << ", " << elevation_ << "]" << endl; }
...
public:
    ...
    void addText(double val, const Colour& colour, double height) { addText(tostring(val), colour, height); }
    ...
    const MagFont& getFont() { return nice_.empty() ? font_ : nice_.back().font(); }
