...
protected:
    ...
    virtual void print(ostream& out) const override { out << "GeoBoxMatrixHandler"; }
};
...
public:
    ...
    int rows() const override { return rowIndex_.size(); }
    int columns() const override { return columnIndex_.size(); }

    double operator()(int row, int column) const override { return matrix_(rowIndex(row), columnIndex(column)); }
    double column(int row, int column) const override { return matrix_.column(rowIndex(row), columnIndex(column)); }
    double row(int row, int column) const override { return matrix_.row(rowIndex(row), columnIndex(column)); }
    double regular_row(int row) const override { return matrix_.regular_row(rowIndex(row)); }
    double regular_column(int column) const override { return matrix_.regular_column(columnIndex(column)); }
