...
void CartesianTransformation::adjustXAxis(Layout& layout) const {
    ...
    if (dataMinX_ == x_->min() && dataMaxX_ == x_->max())
        return;
    if (dataReferenceX_ == "")
        niceAdjust(dataMinX_, dataMaxX_);
    MagLog::dev() << "CartesianTransformation::adjustXAxis()--->[" << dataMinX_ << ", " << dataMaxX_ << "]\n";

    if (dataMaxX_ - dataMinX_ < 4)
        dataMaxX_ = dataMinX_ + 4;  // this needs to be done with more care later!
...
void CartesianTransformation::adjustYAxis(Layout& layout) const {
    ...
    if (dataMinY_ == y_->min() && dataMaxY_ == y_->max())
        return;
    if (dataReferenceY_ == "")
        niceAdjust(dataMinY_, dataMaxY_);
    MagLog::dev() << "CartesianTransformation::adjustYAxis()--->[" << dataMinY_ << ", " << dataMaxY_ << "]\n";
    if (dataMaxY_ - dataMinY_ < 4)
        dataMaxY_ = dataMinY_ + 4;  // this needs to be done with more care later!
    // Should become a user option...
...
MAGICS_NO_EXPORT void CartesianTransformation::operator()(Layout&) const {
    /*
    if (layout["drawing_area"]) layout["drawing_area"]->setCoordinates(x_->minpc(), x_->maxpc(), y_->minpc(),
    y_->maxpc()); if (layout["top_comment_box"]) layout["top_comment_box"]->setCoordinates(x_->minpc(), x_->maxpc(),
    -10, 10); if (layout["bottom_comment_box"]) layout["bottom_comment_box"]->setCoordinates(x_->minpc(), x_->maxpc(),
    -10, 10); if (layout["left_comment_box"]) layout["left_comment_box"]->setCoordinates(-10, 10, y_->minpc(),
    y_->maxpc()); if (layout["right_comment_box"]) layout["right_comment_box"]->setCoordinates(-10, 10, y_->minpc(),
    y_->maxpc()); if (layout["title"]) layout["title"]->setCoordinates(0, 100, 0, 100); if (layout["legend"])
    layout["legend"]->setCoordinates(0, 100, 0, 100);
    */
}
...
void CartesianTransformation::visit(MetaDataVisitor& visitor, double left, double top, double width, double height,
