...
class UserPoint {
public:
    UserPoint(double lon, double lat, double val = 0, bool missing = false, bool border = false) :
        y_(lat),
        x_(lon),
        height_(0),
        value_(val),
    ...
    bool wrapPB(const UserPoint& p) const { return abs(x_ - p.x_) > 180; }

    bool operator==(const UserPoint& other) const {
        return (same(other.x_, x_) && same(other.y_, y_) && same(other.height_, height_) && same(other.value_, value_));
    }
    bool operator!=(const UserPoint& other) const {
        return (!same(other.x_, x_) || !same(other.y_, y_) || !same(other.height_, height_) ||
    ...
    UserPoint left() const { return UserPoint(x_ - 360, y_, value_, height_); }

    UserPoint right() const { return UserPoint(x_ + 360, y_, value_, height_); }
