...
void PolarStereographicProjection::revert(const vector<std::pair<double, double> >& in,
...
void PolarStereographicProjection::smallestBoundingBox(double& xmin, double& ymin, double& xmax, double& ymax) const {
    ...
    for (vector<std::pair<double, double> >::iterator point = geo.begin(); point != geo.end(); ++point) {
        ...
        if (xmax < point->first)
            xmax = point->first;
        if (ymin > point->second)
            ymin = point->second;
        if (ymax < point->second)
            ymax = point->second;
...
void PolarStereographicProjection::boundingBox(double& xmin, double& ymin, double& xmax, double& ymax) const {
    ...
    MagLog::dev() << " Projection bounding box-->[" << xmin << ", " << xmax << ", " << ymin << ", " << ymax << "]"
                  << endl;
}
...
void PolarStereographicProjection::labels(const LabelPlotting& label, DrawingVisitor&) const {
    ...
    for (float i = 0; i < 360; i += 60.) {
        if (find(grid.begin(), grid.end(), i) != grid.end() || find(grid.begin(), grid.end(), i - 360) != grid.end())
...
*/
void PolarStereographicProjection::visit(MetaDataVisitor& visitor, double left, double top, double width, double height,
