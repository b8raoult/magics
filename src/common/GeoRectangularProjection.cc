...
void GeoRectangularProjection::revert(const vector<std::pair<double, double> >& input,
                                      vector<std::pair<double, double> >& output) const {
...
void GeoRectangularProjection::boundingBox(double& xmin, double& ymin, double& xmax, double& ymax) const {
...
void GeoRectangularProjection::gridLongitudes(const GridPlotting& grid) const {
    ...
    for (vector<double>::const_iterator lon = longitudes.begin(); lon != lon_end; ++lon) {
        ...
        for (double lat = min; lat <= max + step; lat += step) {
            (lat > max) ? poly.push_back((*this)(UserPoint(*lon, max))) : poly.push_back((*this)(UserPoint(*lon, lat)));
        }
        grid.add(poly);
    }
}
