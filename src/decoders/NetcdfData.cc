...
void TypedAccessor<F, T>::operator()(vector<T>& to, vector<size_t>& start,
                                     vector<size_t>& edges,
                                     NetVariable& var) const {
  vector<F> from(to.size());
  var.get(from, start, edges);
  // Convert the data....
  std::transform(from.begin(), from.begin() + to.size(), to.begin(),
                 Convertor<F, T>(var));
}

template <class F, class T>
void TypedAccessor<F, T>::get(vector<F>& from, vector<size_t>& start,
                              vector<size_t>& edges, NetVariable& var) const {
...
Netcdf::Netcdf(const string& path, const string& method) : file_(-1) {
  ...
  for (int v = 0; v < num_var; v++) {
    ...
    if (isVariable(file_, var_ids[v]))
      dataset_.insert(
          std::make_pair(name, NetVariable(name, var_ids[v], this, method)));
  }
  ...
  for (int v = 0; v < num_atts; v++) {
    ...
    string name(tmp);
    attributes_.insert(
        std::make_pair(name, NetAttribute(name, file_, NC_GLOBAL)));
  }
...
void Netcdf::print(ostream& out) const {
  ...
  out << "Variables: "
      << "\n";
  for (map<string, NetVariable>::const_iterator var = variables_.begin();
       var != variables_.end(); ++var) {
    out << (*var).second;
  }
  out << "Dataset: "
      << "\n";
  for (map<string, NetVariable>::const_iterator var = dataset_.begin();
       var != dataset_.end(); ++var) {
    out << (*var).second;
  }
}

NetDimension::NetDimension(Netcdf* netcdf, const string& name, int index,
                           int variable)
    : parent_(netcdf),
      name_(name),
      first_(0),
...
int NetDimension::value(const string& val) {
  if (variable_ != -1) {
    // int index = Index::get(variable_->type(), val, variable_->values(),
    // variable_->num_vals());
  ...
  int index = atoi(val.c_str());
  MagLog::warning() << " Could not find variable return index instead " << index
                    << endl;
  return index;
}
...
void NetDimension::last(const string& val) {
  ...
  if (last < first_) {
    MagLog::warning() << "last position (" + val +
...
NetVariable::NetVariable(const string& name, int id, Netcdf* parent,
                         const string& method)
  ...
  for (int d = 0; d < num_dims; d++) {
    ...
    dimensions_.insert(
        std::make_pair(name, NetDimension(parent_, name, d, var)));
...
int find(const T& value, vector<T>& values) {
  if (values.size() == 1 && values[0] == value) return 0;
...
double NetVariable::getDefaultMissing() {
  if (type() == NC_DOUBLE) return NC_FILL_DOUBLE;
  if (type() == NC_BYTE) return NC_FILL_BYTE;
...
string Netcdf::detect(const string& var, const string& type) const {
  ...
  map<string, map<string, vector<string> > >::iterator checks =
      guesser.guess_.find(type);
  if (checks == guesser.guess_.end()) return "";

  for (map<string, vector<string> >::iterator check = checks->second.begin();
       check != checks->second.end(); ++check) {
    vector<string> values = check->second;
    for (vector<string>::iterator dim = dimensions.begin();
         dim != dimensions.end(); ++dim) {
      string value = getVariable(*dim).getAttribute(check->first, string(""));

      for (vector<string>::iterator v = values.begin(); v != values.end();
           ++v) {
