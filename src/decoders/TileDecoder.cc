...
string TileDecoder::projection() {
  map<string, string> projections = {{"cylindrical", "4326"},
                                     {"EPSG:4326", "4326"},
                                     {"EPSG:32661", "32661"},
                                     {"EPSG:32761", "32761"}};
  auto proj = projections.find(projection_);
  if (proj != projections.end()) return proj->second;
  return "none";
}
...
string TileDecoder::weights() {
  ...
  out << parent << "/weight-" << grid_ << "-" << projection()
      << "-z" + tostring(z_) << ".nc";
  return out.str();
}
...
string TileDecoder::positions() {
  ...
  out << parent << "/wind-" << grid_ << "-" << projection()
      << "-z" + tostring(z_) << ".nc";
...
string TileDecoder::positions_symbols() {
  ...
  out << parent << "/symbol-" << grid_ << "-" << projection()
      << "-z" + tostring(z_) << ".nc";
...
bool TileDecoder::ok() {
  ...
  if (handle_ == NULL) {
    MagLog::error() << "ERROR: unable to create handle from file" << file_name_
                    << endl;
    return false;
  }
  char val[1024];
...
PointsHandler& TileDecoder::points(const Transformation& t, bool) {
...

  {
    ...
    codes_get_double_elements(f, "values", &index.front(), index.size(),
                              &values.front());
...
void TileDecoder::scaling_offset(codes_handle* f, double& scaling,
                                 double& offset) {
...
void TileDecoder::decode() {
  ...
  if (bbox.empty()) {
    ...
    FILE* in = fopen(file_name_.c_str(), "r");
    if (!in) {
      MagLog::error() << "ERROR: unable to create handle from file"
                      << file_name_ << endl;
      return;
    }
  ...
  for (auto l = dindex.begin(); l != dindex.end(); ++l) {
    if (*l > -1) iindex.push_back(*l);
  }

  codes_get_double_elements(handle_, "values", &iindex.front(), iindex.size(),
                            &dvalues.front());
